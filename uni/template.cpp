...
private:
	...
	Node<T>* _getSuccessor(Node<T>* node){
		...
		if(tmp != nullptr) return _min(node);
		tmp = tmp->parent; 
	...
	Node<T>* _getPredecessor(Node<T>* node){
		...
		if(tmp!= nullptr) return _max(node);
...
public:
	...
	BST& erase(T key)
	{
		...
		{
			Node<T>* par = x->parent;
			if(key < par->key) par->left = nullptr;
...
int main(){
	...
	{	
		...
		if(type == "int"){
			...
			PrintListOrder<int>(out,order,bst);
		}
		if(type == "bool"){
			BST<int> bst;
			int b;
		...
		if(type == "double"){
			...
			PrintListOrder<double>(out,order,bst);
		}
		if(type == "char"){
			BST<char> bst;
			char b;
			...
			PrintListOrder<char>(out,order,bst);
		}	
		out	<<endl;	
	}
	return 0;
